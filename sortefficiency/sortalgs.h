...
/* another quicksort version 
void SortAlgs::quickSort(int len, int *arr) {
	// creat pivot
	...
	// partition array above and below pivot
	while(min < max) {
		while(arr[min] < pivot) {
			min++;
		}
		...
		// swap etries at min and max
		int temp = arr[min];
		arr[min] = arr[max];
		arr[max] = temp;
	}
	// use recursun to call partition on partition
	quickSort(min, arr);
	quickSort(len-min-1, &(arr[min+1]));	
}
*/
#endif
